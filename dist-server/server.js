...
var init_db = __esm({
  "src/server/core/db.ts"() {
    dbPath = path.join(process.cwd(), "data", "healthmate.db");
...
var ocr_parse_lab_report = aiTool({
...
var query_lab_norms = aiTool({
  ...
  execute: async ({ indicators }) => {
    const normsPath = path2.join(process.cwd(), "data", "lab_norms.json");
    const db2 = JSON.parse(fs.readFileSync(normsPath, "utf8"));
...
var search_medical_kb = aiTool({
  ...
  execute: async ({ query }) => {
    const kbPath = path2.join(process.cwd(), "data", "medical_kb.json");
...
var detect_critical_symptoms = aiTool({
...
var generate_doctor_questions = aiTool({
...
var generate_wechat_message_for_doctor = aiTool({
...
var query_member_profile = aiTool({
...
var compare_with_previous = aiTool({
...
var analyze_indicator_trend = aiTool({
...
var search_family_history = aiTool({
...
var search_personal_notes = aiTool({
...
var ocr_parse_prescription = aiTool({
  ...
  execute: async ({ image_path }) => {
    try {
      if (!fs.existsSync(image_path)) return "File not found.";
      const worker = await tesseract.createWorker("chi_sim+eng");
      const { data: { text } } = await worker.recognize(image_path);
      await worker.terminate();
...
var parse_prescription_text = aiTool({
...
var check_drug_interaction = aiTool({
  ...
  execute: async ({ drugs }) => {
    const kbPath = path2.join(process.cwd(), "data", "drug_interactions_kb.json");
...
var recommend_department = aiTool({
...
var generate_visit_preparation = aiTool({
...
var detect_anomaly_in_history = aiTool({
...
var predict_medication_runout = aiTool({
...
var cross_indicator_correlation = aiTool({
...
var recommend_followup_visit = aiTool({
...
var detect_seasonal_health_risk = aiTool({
...
var generate_proactive_alert = aiTool({
...
var estimate_action_priority = aiTool({
...
var generate_weekly_report = aiTool({
...
var parse_doctor_dialogue = aiTool({
...
var generate_annual_report = aiTool({
...
var parse_multipage_pdf = aiTool({
...
var extract_all_indicators = aiTool({
...
var find_critical_anomalies = aiTool({
...
var generate_health_summary = aiTool({
...
var loginLimiter = rateLimit({ windowMs: 15 * 60 * 1e3, max: 5, message: { error: "Too many login attempts, please try again after 15 minutes" } });
var emailLimiter = rateLimit({ windowMs: 60 * 60 * 1e3, max: 3, message: { error: "Too many emails sent, please try again later" } });
var registerLimiter = rateLimit({ windowMs: 60 * 60 * 1e3, max: 10, message: { error: "Too many registrations from this IP" } });
...
router.post("/password-reset/confirm", async (req, res) => {
  ...
  try {
    const decoded = jwt.verify(reset_session_token, JWT_SECRET);
...
var uploadDir = path3.join(process.cwd(), "data", "uploads");
...
async function startServer() {
  ...
  const PORT = 3e3;
  app.use(cors());
  app.use(express12.json({ limit: "50mb" }));
